...
### 2.4 Dead ends and major challenges

- **Seamless flicker-free rendering.** I spent significant effort on a "seamless" approach that tried to avoid all visible redraw artifacts across frames. The approach did not pan out, and the final pipeline uses the simpler `\x1b[H` cursor-home + `\x1b[?25l` hide-cursor sequence (`printer.rs`) to overwrite the previous frame in place. This is good enough in practice but is not the more sophisticated solution I originally wanted.
