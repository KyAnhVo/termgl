...
- **`.obj` / `.mtl` import was much, much harder than it looked.** A naive read of the spec ("vertices on `v` lines, faces on `f` lines, materials on `usemtl` lines") undersells the format spectacularly. The actual importer (`import_obj` plus `import_mesh_line_process` plus `import_mtl` in `mesh.rs`) ended up several hundred lines of deeply-nested branching because each of the following has to be handled and none of them is skippable:
    - **Four distinct face-index formats can appear in the same file:** `f v`, `f v/vt`, `f v//vn`, and `f v/vt/vn`. Detecting `v//vn` in particular relies on noticing that splitting `"1//2"` on `/` yields `["1", "", "2"]` — an empty middle string, not a missing field — and dispatching on that empty slot. Getting any of the four cases wrong silently corrupts the mesh.
    - **Missing UVs and normals have to be fabricated on the fly.** When a face omits a UV index, the importer pushes a default `(0,0)` into the UV array and references it by index. When a face omits a normal index, the importer computes a face normal from the cross product of two of the triangle's edges, pushes it, and references it. Each fabrication has to keep the shared vertex/UV/normal index space consistent across every later face that touches the same data.
    - **OBJ allows arbitrary n-gon faces, not just triangles**, so the importer has to triangulate every face it sees. Ear-clipping handles convex and most concave polygons but stalls on the near-degenerate / non-strictly-convex faces real-world `.obj` files contain — which is what motivated the triangle-fan parachute described above.
    - **Material state is mid-file mutable.** A single `.obj` can switch materials via `usemtl` repeatedly, so one `.obj` plus one or more `.mtl` files can produce *multiple* `Mesh` objects — one per material — all sharing a common vertex/UV/normal pool. The importer maintains a `HashMap<String, Vec<VertexIndices>>` keyed on material name and only emits a `Mesh` per non-empty bucket at EOF. The user can also pass several `.mtl` files at once, with the constraint that material names must not collide.
    - **MTL parsing needs an explicit end-of-file flush.** The `.mtl` parser is a running state machine that commits the previous material into the map only when it sees the next `newmtl` directive. Without a manual flush after the loop ends, the *last* material in every `.mtl` file silently disappears — exactly the kind of bug that doesn't show up until someone uses a single-material `.mtl`.
    - **OBJ indexing is 1-based, Rust slicing is 0-based.** Every parsed face index has a `- 1` after it, and the offset has to be applied consistently across all four face-format branches and across both the "with `.mtl`" and "without `.mtl`" code paths — which currently exist as parallel near-duplicates of the same logic. A `DEFAULT` material is pre-loaded into the map specifically so faces that appear before any `usemtl` don't crash on a missing key.

    Most of these are not in the OBJ spec the way they show up in real files; they're things you only learn by feeding the importer a few dozen meshes from different sources and watching it explode in different ways each time.
...
The following are concrete next steps for the library, in roughly the order I would attack them:

- **Function-pointer (closure-free) input system.** Proposal goal 6 was abandoned because every clean closure-based design path (`Box<dyn Fn>`, trait objects, etc.) measured slow enough to hurt frame time. The right path forward is a `HashMap<KeyCode, fn(&mut Context)>` — a plain `fn` pointer table indexed by keycode, with state threaded through an explicit `Context` argument rather than captured. This is the C-callback-table idiom in Rust dress; it gets back O(1) dispatch with no virtual-call overhead and should fit comfortably inside the per-frame budget.
- **Additional print backends, starting with a border / edge backend.** Proposal goal 5. A border printer would run a Sobel-style high-pass filter over the depth or color buffer and emit only silhouette / edge pixels using box-drawing or line-art glyphs (`│ ─ ╱ ╲`). It's useful on its own for an ASCII line-art aesthetic and as a building block for compositing TermGL output on top of a real text buffer (overlays, HUDs, tmux panes). Beyond border, the natural other backends to add are quarter-block (4× density vs. half-block) and Braille (8× density) for the modern-terminal regime described in the postscript below.
- **Refactor `Pipeline3D` to render arbitrary multi-mesh scenes.** Today the pipeline takes one mesh at a time and the demos manually loop, calling `render_mesh` per object. Lifting this into a proper `Scene` / `World` abstraction that owns a `Vec<Mesh>` together with its lights, cameras, and transforms would let the pipeline batch transforms, share matrices once per frame, and (eventually) parallelize across meshes or screen tiles with Rayon. It also opens the door to standard scene-graph features — hierarchy, instancing, per-mesh visibility.
- **Implicit geometry support.** Right now every renderable object is a triangle mesh. Implicit surfaces — signed distance functions (sphere, plane, torus, CSG combinations) — can be added in one of two ways: tessellated up front via marching cubes / dual contouring and then fed through the existing rasterizer, or ray-marched per pixel through an alternate render path that bypasses rasterization entirely. The first is the lower-cost integration; the second is much more interesting visually (true CSG, soft blends, fractals) and is essentially a separate small renderer living next to the triangle one.
- **A first-class line-drawing primitive.** Currently the only way to draw a line is to fake it as a thin triangle strip — orbit lines in the solar system demo are flat annuli with a small radial delta, which is wasteful and aliases poorly at low resolution. A real Bresenham / DDA line in screen space (with depth-buffer participation so it occludes correctly) would clean up wireframes, axis indicators, debug visualization, and the orbit lines themselves, and is a small amount of code on top of the existing `draw_pixel`.
